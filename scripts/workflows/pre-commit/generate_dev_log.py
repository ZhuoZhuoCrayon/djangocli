...
def get_git_username() -> str:
    ...
    raise ValueError(f"get git username error: confirm your config.")
...
def generate_dev_log(dev_log_root: str, commit_message: str) -> Dict[str, Any]:
    ...
    user_dev_log_yaml_path = os.path.join(dev_log_root, rc_version, f"{git_username}.yaml")
    commit_msg_parse_result = parse_commit_message(commit_message)

    is_record = check_is_record(
        dev_log_yaml_paths=[user_dev_log_yaml_path], commit_msg_parse_result=commit_msg_parse_result
    )
    if is_record:
    ...
    yaml_info[datetime.now().strftime("%Y-%m-%d %H:%M")] = {
        commit_msg_parse_result["commit_type"]: [DoubleQuotedScalarString(commit_msg_parse_result["commit_content"])]
    }
