...
def op_yaml(
    yaml_path: str, keyword_path: str, op: str, value: Optional[Union[Dict, List, str, int, float]] = None
    ...
    kw_result = find_and_op_yaml_info(
        yaml_info=yaml_info, kw_path_steps=parse_keyword_path(keyword_path=keyword_path), op=op, value=value
    )
...
if __name__ == "__main__":
    ...
    result = op_yaml(
        yaml_path=params["yaml-path"], keyword_path=params["keyword-path"], op=params["op"], value=params["value"]
    )
